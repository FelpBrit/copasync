document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelector('#exportAll')?.addEventListener('click',()=>{
    const calGames = games.filter(g=>CopaSync.isIn(CopaSync.LS.calendar,g.id));
    if(!CopaSync.getUser()) return CopaSync.showToast('Entre com um perfil local para exportar o calendário.');
    if(!calGames.length) return CopaSync.showToast('Adicione pelo menos um jogo ao calendário.');
