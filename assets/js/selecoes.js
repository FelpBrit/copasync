document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelector('#teamGroupFilter').innerHTML = `<option value="todos">Todos os grupos</option>${groups.map(g=>`<option value="${g}">${g}</option>`).join('')}`;
  const render = () => {
    const list = teams.filter(t => (state.grupo==='todos'||t.grupo===state.grupo) && t.nome.toLowerCase().includes(state.busca.toLowerCase()));
    document.querySelector('#teamsList').innerHTML = list.length ? list.map(t=>`<article class="team-card ${CopaSync.isIn(CopaSync.LS.favTeams,t.id)?'favorite':''}"><div class="team-header"><div><div class="flag">${t.bandeira}</div><b>${t.nome}</b><p class="muted">${t.grupo} · ${t.continente}</p></div><button class="pill-btn fav ${CopaSync.isIn(CopaSync.LS.favTeams,t.id)?'active':''}" data-fav-team="${t.id}">${CopaSync.isIn(CopaSync.LS.favTeams,t.id)?'Favorita':'Favoritar'}</button></div><a class="ghost-btn" href="jogos.html?selecao=${encodeURIComponent(t.nome)}">Ver jogos da seleção</a></article>`).join('') : CopaSync.empty('Nenhuma seleção encontrada','Tente buscar por outro nome ou grupo.');
    document.querySelectorAll('[data-fav-team]').forEach(btn=>btn.addEventListener('click',()=>{ if(!CopaSync.requireUser('favoritar seleções')) return; CopaSync.toggleList(CopaSync.LS.favTeams,btn.dataset.favTeam); render(); }));
  };
