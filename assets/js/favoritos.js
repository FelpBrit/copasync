document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelector('#favTeamsBox').innerHTML = favTeams.length ? favTeams.map(t=>`<article class="team-card favorite"><div class="flag">${t.bandeira}</div><b>${t.nome}</b><span class="muted">${t.grupo}</span></article>`).join('') : CopaSync.empty('Nenhuma seleção favorita','Favorite seleções para criar o seu torneio personalizado.','selecoes.html','Escolher seleções');
